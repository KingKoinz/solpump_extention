...
class CrashMonitor {
  ...
  async saveResult(result) {
    // Store only in memory to avoid chrome.storage context invalidation issues
    // Results are kept in this.results array (max 500)
    console.log('[Solpump Analyzer] Result stored in memory');
  }

  async loadHistoricalData() {
    // Load from memory only - chrome.storage causes context invalidation
    console.log('[Solpump Analyzer] Data loading from memory only');
  }
  ...
  handlePopupMessage(msg, sendResponse) {
    try {
      switch(msg.action) {
        case 'getStats':
          try {
            ...
            console.error('[Solpump] Error getting stats:', e);
        ...
        case 'generateDemoData':
          try {
            console.log('[Solpump Analyzer] Generating demo data...');
            const demoResults = this.generateDemoResults(50);
            this.results = demoResults;

            const demoAnalysis = this.performAnalysis();
            sendResponse({ success: true, stats: demoAnalysis, count: 50 });
          } catch (e) {
            console.error('[Solpump] Error generating demo:', e);
            sendResponse({ success: false, error: e.message });
          }
          break;
        ...
        case 'clearData':
          try {
            ...
            console.error('[Solpump] Error clearing data:', e);
            sendResponse({ success: false, error: e.message });
          }
          break;
        ...
        case 'getHistory':
          try {
            sendResponse({ success: true, history: this.results });
          } catch (e) {
            console.error('[Solpump] Error getting history:', e);
            sendResponse({ success: false, error: e.message });
          }
          break;
      ...
      console.error('[Solpump] Unexpected error in handlePopupMessage:', e);
      try {
        sendResponse({ success: false, error: 'Extension error: ' + e.message });
      } catch (sendErr) {
        console.error('[Solpump] Could not send error response:', sendErr);
      }
    }
  }

  generateDemoResults(count) {
    const results = [];
    const now = Date.now();

    for (let i = 0; i < count; i++) {
      // Generate realistic crash multipliers
      // Most are 1.x-2.x, some are higher
      const rand = Math.random();
      let multiplier;

      if (rand < 0.5) {
        multiplier = 1 + Math.random() * 1.5; // 1.0-2.5
      } else if (rand < 0.8) {
        multiplier = 1.5 + Math.random() * 1.5; // 1.5-3.0
      } else if (rand < 0.95) {
        multiplier = 2 + Math.random() * 3; // 2.0-5.0
      } else {
        multiplier = 3 + Math.random() * 7; // 3.0-10.0
      }

      results.push({
        multiplier: parseFloat(multiplier.toFixed(2)),
        timestamp: now - (count - i) * 30000, // 30 seconds apart
        betAmount: 0.01,
        result: 'busted'
      });
    }
